"""
Re-evaluate FullModel checkpoint with corrected metrics.
Extracts FlowChain weights from old-architecture checkpoint,
evaluates unconditionally (consistent with original evaluation).
Usage: python scripts/reeval_fullmodel.py
"""
import sys, torch
sys.path.insert(0, '.')
...
# Create FlowChain-only model and load matching weights
...
# Filter: keep only FlowChain weights, remap prefix
# Checkpoint: flow_chain.model.xxx  (TrafficPerceptionModel.flow_chain.model)
# FlowChainBase: predictor.model.xxx
flow_state = {}
...
print(f'FullModel: ADE={m["ADE"]:.2f}, FDE={m["FDE"]:.2f}, NLL={m["NLL"]:.2f}')
